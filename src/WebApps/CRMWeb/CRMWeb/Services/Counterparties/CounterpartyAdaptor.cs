...
public class CounterpartyAdaptor : DataAdaptor
{
    private readonly ICounterpartyService _counterpartyService;

    public CounterpartyAdaptor()
    {
    ...
    public override async Task<object> ReadAsync(DataManagerRequest dataManagerRequest, string? key = null)
    {
        ...
        var pageIndex = pageSize == 0 ? 0 : dataManagerRequest.Skip / pageSize;

        var response = await _counterpartyService.GetCounterparties(pageIndex, pageSize);

        if (response.IsSuccessStatusCode)
        {
            IEnumerable<CounterpartyModel> dataSource = response.Content!.Counterparties.Data;
            ...
            var count = response.Content!.Counterparties.Count;
            ...
            if (dataManagerRequest.Group != null)
            {
                ...
                {
                    resultData = DataUtil.Group<CounterpartyModel>(resultData, group, dataManagerRequest.Aggregates, 0, dataManagerRequest.GroupByFormatter);
                }
    ...
    {
        var insert = (CounterpartyFullModel)data;

        var response = await _counterpartyService.CreateCounterparty(new CreateCounterpartyRequest(insert));

        if (response.IsSuccessStatusCode)
        {
            CreateCounterpartyResponse createCounterpartyResponse = response.Content!;
            return createCounterpartyResponse;
        }
        else
        {
    ...
    {
        var response = await _counterpartyService.UpdateCounterparty(new UpdateCounterpartyRequest((CounterpartyFullModel)data));
        if (response.IsSuccessStatusCode)
        {
            UpdateCounterpartyResponse updateResponse = response.Content!;
    ...
    {
        var response = await _counterpartyService.DeleteCounterparty((Guid)data);
        if (response.IsSuccessStatusCode)
        {
            DeleteCounterpartyResponse deleteResponse = response.Content!;
